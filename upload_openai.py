"""
Enhanced script to upload documents from various file formats to Qdrant using OpenAI embeddings
...
def create_collection_openai():
    """Create collection optimized for OpenAI embeddings (1536 dimensions)"""
    try:
        ...
        if QDRANT_COLLECTION not in collection_names:
            client.create_collection(
                ...
                vectors_config=models.VectorParams(
                    size=1536,  # OpenAI text-embedding-ada-002 produces 1536-dimensional embeddings
...
def upload_documents(documents):
    """Upload a list of documents to Qdrant using OpenAI embeddings"""
    if not documents:
        print("No documents to upload")
        return
    
    print(f"Uploading {len(documents)} document chunks with OpenAI embeddings...")
    
    points = []
    for i, doc in enumerate(documents):
        print(f"Generating embedding for document {i+1}/{len(documents)}: {doc['title']}")
        
        # Generate embedding using OpenAI
        embedding = generate_openai_embedding(doc['text'])
        if embedding is None:
            print(f"Skipping document {i+1} due to embedding error")
            continue
    ...
    batch_size = 50  # Smaller batches for OpenAI rate limits
...
def test_search(query="Facebook Ads targeting"):
    ...
    try:
        ...
        for i, hit in enumerate(search_result):
            ...
            print(f"   Embedding Model: {hit.payload.get('embedding_model', 'Unknown')}")
...
if __name__ == "__main__":
    print("=== OpenAI Enhanced Document Upload Tool ===")
    print("Supported formats: .txt, .md, .pdf, .docx")
    print("Using OpenAI text-embedding-ada-002 for embeddings")
    print()
